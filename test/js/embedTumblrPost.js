...
<HEAD>
    <SCRIPT>
        let embedFeaturedTumblrPosts = new embedTumblrPosts(
            {
                debug : 0,
                blog_id : 't:y0T-4LsVtmTl7vk9cPzUiQ',
                limit : 2,
...
<BODY>
    ...
    <script>
        ...
        scr.src="https://demo.tumblr.com/api/read/json?tagged=featured&callback=embedFeaturedTumblrPosts.embedPosts&rand="+Math.randInt(100000,999999);
        var attr=document.createAttribute('onload');
        attr.value="embedTumblrPosts('RecentPosts', 5,null,'featured');";
        scr.setAttributeNode(attr);
        document.getElementById("RecentPosts").appendChild(scr);
    </script>    
    <script>
        var scr=document.createElement('script');
        scr.src="https://demo.tumblr.com/api/read/json?callback=embedRecentTumblrPosts.embedPosts&rand="+Math.randInt(100000,999999);
        var attr=document.createAttribute('onload');
        attr.value="embedTumblrPosts('RecentPosts', 5,null,'featured');";
        scr.setAttributeNode(attr);
        document.getElementById("RecentPosts").appendChild(scr);
...
let embedTumblrPosts = function(params_) {
    ...
    this.TOP_DOM_ELEMENT = typeof params_.top_dom_element_id != 'undefined' ? document.getElementById(params_.top_dom_element_id) : null;
    this.FANCY_POSTS = typeof params_.fancy_posts != 'undefined' ? params_.fancy_posts : null;
};
// --------------------------------------------------------
embedTumblrPosts.prototype.embedPlainPosts = function(post) {
...
embedTumblrPosts.prototype.embedPosts = function(tumblr_api_read) {
    try {this.POSTS = tumblr_api_read.posts;}
    catch(err) {console.log('tumblr_api_read data object is undefined!');void(0);}
    this.LEN = this.POSTS.length;
    if(this.DEBUG){console.log('++embedPosts starts');}
    for (var i = 0; i < this.LEN; ++i) {
        ...
        if (this.DEBUG) {console.log('post tag array:' + post.tags);}
        if(JSON.stringify(post.tags).searchList(this.BLACKLISTED_HASHTAGS)!==-1) {
            if (this.DEBUG) {console.log('found blacklisted hashtag');}
            continue;
        }
        ...
        this.embedPlainPosts();
        this.TOP_DOM_ELEMENT.appendChild(recent);
        if(!this.FANCY_POSTS){
            var scr=document.createElement('script');
            scr.src="https://assets.tumblr.com/post.js";
            this.TOP_DOM_ELEMENT.appendChild(scr);
        }
        if(this.DEBUG){console.log('--embedPosts');}
        if(--this.LIMIT===0){
            if(this.DEBUG){console.log('specified limit reached, exiting loop');}
            break;
        }
    }
    if(this.DEBUG){console.log('++embedPosts ends');}
}
